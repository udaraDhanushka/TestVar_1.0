...
export default function SingleFlashcard({ flashcard, onGotIt, onKnewIt, disabled = false }: SingleFlashcardProps) {
  ...
  return (
    <div className="flex items-center justify-center">
      <div className={`cursor-pointer perspective-1000 ${disabled ? 'pointer-events-none' : ''}`} style={{ width: '800px' }} onClick={handleClick}>
        <div
          className={`relative w-full transform-style-3d transition-transform duration-500 ${isFlipped ? 'rotate-y-180' : ''
            }`}
        >
          {/* Front of card */}
